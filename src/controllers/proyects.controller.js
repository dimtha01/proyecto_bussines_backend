...
export const getProyectsAllRequisition = async (req, res) => {
  ...
  SELECT 
    ...
    p.duracion,
    p.observaciones,
    p.monto_estimado_oferta_cliente,
    p.monto_estimado_oferta_cerrado_sdo,
    ec.nombre AS estatus_comercial,
    
    -- ✅ COSTO REAL (subconsulta corregida)
    COALESCE((SELECT SUM(co.costo) 
              FROM costos_proyectos co 
              WHERE co.id_proyecto = p.id), 0) AS costo_real,
    
    -- ✅ FACTURADO (estatus 6)
    COALESCE((SELECT SUM(af_financiero.monto_usd)
              FROM avance_financiero af_financiero
              INNER JOIN estatus_proceso ep ON af_financiero.id_estatus_proceso = ep.id_estatus
              WHERE af_financiero.id_proyecto = p.id AND ep.id_estatus = 6), 0) AS facturado,
    
    -- ✅ POR VALUAR (estatus 4)
    COALESCE((SELECT SUM(af_financiero.monto_usd)
              FROM avance_financiero af_financiero
              INNER JOIN estatus_proceso ep ON af_financiero.id_estatus_proceso = ep.id_estatus
              WHERE af_financiero.id_proyecto = p.id AND ep.id_estatus = 4), 0) AS por_valuar,
    
    -- ✅ POR FACTURA (estatus 5)
    COALESCE((SELECT SUM(af_financiero.monto_usd)
              FROM avance_financiero af_financiero
              INNER JOIN estatus_proceso ep ON af_financiero.id_estatus_proceso = ep.id_estatus
              WHERE af_financiero.id_proyecto = p.id AND ep.id_estatus = 5), 0) AS por_factura,
    ...
    -- ✅ ✅ FECHAS MÁXIMAS CORREGIDAS (subconsultas independientes)
    COALESCE((SELECT MAX(af_fisico.avance_real) 
              FROM avance_fisico af_fisico 
              WHERE af_fisico.id_proyecto = p.id), 0) AS avance_real_maximo,
    COALESCE((SELECT MAX(af_fisico.avance_planificado) 
              FROM avance_fisico af_fisico 
              WHERE af_fisico.id_proyecto = p.id), 0) AS avance_planificado_maximo,
    (SELECT MAX(af_fisico.fecha_fin) 
     FROM avance_fisico af_fisico 
     WHERE af_fisico.id_proyecto = p.id) AS ultima_fecha_fin_avance_fisico
